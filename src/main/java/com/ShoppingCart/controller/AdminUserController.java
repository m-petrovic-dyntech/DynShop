...
public class AdminUserController extends ControllerUtil {
	...
	public ModelAndView adminDeleteCustomer(ModelAndView modelAndView, @PathVariable(value = "id") int id,
		...
		modelAndView.setViewName(getRedirectLink("redirect:/admin/panel/users", request, Arrays.asList()));
		return modelAndView;
	}
	...
	public ModelAndView adminAddCustomer(ModelAndView modelAndView,
			@ModelAttribute("customerAddModel") Customer customer, HttpServletRequest request, HttpSession session) {
	...
	public ModelAndView adminEditCustomer(ModelAndView modelAndView,
			@ModelAttribute("customerEditModel") Customer customer, HttpServletRequest request, HttpSession session) {
		...
		for (String s : parameterMap.get("customer_roles")) { // iscitavanje
																// stare role
			roles.add(s);
		}
		...
		modelAndView.setViewName(getRedirectLink("redirect:/admin/panel/users", request, Arrays.asList("id",
				"firstName", "lastName", "city", "address", "phone", "password", "customer_roles", "enabled")));
		return modelAndView;
	}
